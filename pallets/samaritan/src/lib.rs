...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		pub fn change_si(
			user_id: &T::AccountId,
			ip_address: &BoundedVec<u8, T::MaxIPAddress>,
		) -> u64 {
			let now: u64 = T::TimeProvider::now().as_secs();

			let nsd: SignInDetail<T> = SignInDetail { ip: ip_address.clone(), time: now };
		...
		pub fn update_sam(
			user_id: &T::AccountId,
			ps: &BoundedVec<u8, T::MaxPseudoNameLength>,
			cid: &BoundedVec<u8, T::MaxCIDLength>,
		) {
			let sam: Samaritan<T> = Samaritan { pseudoname: ps.clone(), cid: cid.clone() };
